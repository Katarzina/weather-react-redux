	...
	render() {
		...
		return (
            ...
            {focusedData.map((list, index) => {
              ...
              return <tr key={index}>
							       <td><Timestamp time={list.dt} key={list.dt} format='time' /></td><td><img src={icon} alt="weather"/></td><td>{list.main.temp > 0 ? "+" : "" }{list.main.temp}°</td>
							       <td>{list.wind.speed} mps</td><td>{list.main.humidity}%</td><td>{list.main.pressure}</td>
 						         </tr>
                    }).filter((date) => {
                        return date
						        })
